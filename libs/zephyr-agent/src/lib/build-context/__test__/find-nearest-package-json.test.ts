...
import { describe, expect, rs, type Mock } from '@rstest/core';
import { find_nearest_package_json } from '../find-nearest-package-json';
import { ZeErrors, ZephyrError } from '../../errors';

// Mocking the functions for testing
rs.mock('node:fs', () => {
  const actualFs = rs.requireActual('node:fs') as typeof nodeFs;
  const accessSync = rs
    .fn()
    .mockImplementation(() => new Error('unexpected call to mock'));
  return {
    ...actualFs,
    accessSync,
    default: { ...actualFs, accessSync },
  };
});
