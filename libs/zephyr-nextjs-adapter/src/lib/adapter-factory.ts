 ...
 * 
 * Allows users to create custom adapters with specific configuration
 * while maintaining compatibility with the standard Zephyr infrastructure.
 */
  ...
  return {
    ...
    modifyConfig: async (nextConfig: NextConfigComplete) => {
      ...
      if (config.enableDetailedLogging) {
        ...
        log.info(`   - Package: ${zephyrConfig.packageInfo?.name}@${zephyrConfig.packageInfo?.version}`)
      ...
      if (nextConfig.webpack) {
        ...
        nextConfig.webpack = (webpackConfig: any, options: any) => {
          ...
          if (result.plugins) {
            ...
            result.plugins = result.plugins.filter((plugin: any) => 
      ...
      try {
        ...
        if (config.customAssetFilter) {
          ...
          if (filteredOutputs.length < originalCount) {
            log.info(`🔍 Filtered ${originalCount - filteredOutputs.length} assets using custom filter`)
          }
        }
        ...
        if (config.excludePatterns && config.excludePatterns.length > 0) {
          ...
          if (filteredOutputs.length < originalCount) {
            log.info(`🚫 Excluded ${originalCount - filteredOutputs.length} assets using exclude patterns`)
          }
        }
        ...
        const legacyBuildContext: BuildContext = {
          routes: {
            headers: ctx.routes.headers.map(route => ({
              source: route.source || '',
              headers: route.headers?.reduce((acc, h) => ({ ...acc, [h.key]: h.value }), {}) || {},
              has: route.has,
        ...
        if (config.customMetadata) {
          (snapshot as any).customMetadata = config.customMetadata
...
export function createDevelopmentAdapter(): NextAdapter {
  return createZephyrAdapter({
    onBuildComplete: async (ctx: BuildContext) => {
      ...
      const outputsByType = ctx.outputs.reduce((acc, output) => {
...
export function createCIAdapter(): NextAdapter {
  ...
  return createZephyrAdapter({
    ...
    onUploadComplete: async (result) => {
      if (isCI) {
        if (result.success) {
          console.log(`::notice::Zephyr upload completed successfully (${result.uploadedAssets} assets)`)
        } else {
