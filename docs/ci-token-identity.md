...
GitLab reads `CI_JOB_TOKEN` as a JWT when possible and extracts `user_email` or `email`. If the token is legacy/non-JWT
...
then commit committer email, then `head_commit`, then `pusher.email`. When no event email exists, it falls back to
GitHub's noreply email shape from `GITHUB_ACTOR_ID` and `GITHUB_ACTOR`/`GITHUB_TRIGGERING_ACTOR`. This requires no
workflow YAML changes beyond setting `ZE_CI_TOKEN`, but GitHub can only attribute users that have the chosen commit or
noreply email registered in Zephyr.
