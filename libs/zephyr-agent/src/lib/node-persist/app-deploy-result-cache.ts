...
export async function getAppDeployResult(
  ...
  await storage;
  return node_persist.getItem(`${StorageKeys.ze_app_deploy_result}:${application_uid}`);
}

export async function removeAppDeployResult(application_uid: string): Promise<void> {
  await storage;
  await node_persist.removeItem(`${StorageKeys.ze_app_deploy_result}:${application_uid}`);
}

export async function getAllDeployedApps(): Promise<string[]> {
  await storage;
  const allKeys = await node_persist.keys();
...
export async function getAllAppDeployResults(): Promise<Record<string, DeployResult>> {
  ...
  await node_persist.forEach((entry) => {
