 ...
 *
 * This example shows how to create a custom adapter with specific configuration
 * and filtering.
 */
...
export default createZephyrAdapter({
  ...
  customAssetFilter: (asset) => {
    ...
    if (
      asset.pathname.includes('/_error') ||
      asset.pathname.includes('/_app')
    ) {
      return false;
    }
  ...
  excludePatterns: ['/test/', '/__debug/', '.test.js', '.spec.js'],
  ...
  onUploadStart: async (snapshot) => {
    console.log(
      `📤 Uploading snapshot ${snapshot.id} with ${snapshot.metadata.totalOutputs} assets`
    );
  },
