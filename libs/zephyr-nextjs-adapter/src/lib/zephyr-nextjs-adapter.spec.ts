...
describe('ZephyrNextJSAdapter', () => {
  ...
  describe('modifyConfig', () => {
    it('should configure Next.js for Zephyr deployment', async () => {
      const mockConfig: NextConfigComplete = {
        experimental: {},
      };

      const result = await zephyrNextJSAdapter.modifyConfig(mockConfig);

      expect(result.output).toBe('standalone');
      expect(result.experimental.esmExternals).toBe(true);
      expect(result.experimental.serverComponentsExternalPackages).toContain(
        'zephyr-agent'
      );
      expect(result.experimental.serverComponentsExternalPackages).toContain(
    ...
    it('should preserve existing experimental config', async () => {
      const mockConfig: NextConfigComplete = {
        experimental: {
    ...
    it('should handle webpack configuration', async () => {
      ...
      expect(
        processedConfig.plugins.some((p: any) => p.constructor.name.includes('Zephyr'))
      ).toBe(false);
    });
  });
  ...
  describe('onBuildComplete', () => {
    ...
    it('should process build context successfully', async () => {
      await expect(
        zephyrNextJSAdapter.onBuildComplete(mockBuildContext)
    ...
    it('should call core functions in correct order', async () => {
      ...
      expect(convertToZephyrAssets).toHaveBeenCalledWith(
        mockBuildContext,
        expect.any(Object)
      );
    ...
    it('should handle errors gracefully', async () => {
      ...
      await expect(zephyrNextJSAdapter.onBuildComplete(mockBuildContext)).rejects.toThrow(
        'Test error'
      );
    });
  });
  ...
  describe('integration', () => {
    it('should work with empty build context', async () => {
      ...
      await expect(
        zephyrNextJSAdapter.onBuildComplete(emptyContext)
      ).resolves.not.toThrow();
    });
    ...
    it('should work with complex build context', async () => {
      ...
      await expect(
        zephyrNextJSAdapter.onBuildComplete(complexContext)
      ).resolves.not.toThrow();
    });
  });
});
