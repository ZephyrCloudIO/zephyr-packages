 ...
 * 
 * The main adapter implementation that integrates with the Next.js build process
 * and creates a single snapshot after the entire build completes.
 */
...
const zephyrNextJSAdapter: NextAdapter = {
  ...
  /**
   * Handle the complete build - this is where we create our single snapshot
   * This runs AFTER all compilation, pre-rendering, and asset generation is finished
   */
  onBuildComplete: async (ctx: {
    ...
    try {
      ...
      log.info('✨ Zephyr Next.js Adapter: Single snapshot created and uploaded successfully!')
