...
export interface ZephyrOTAConfig {
  /**
   * Auth token for private applications.
   * Can also be set via ZE_AUTH_TOKEN environment variable.
   */
  authToken?: string;
  ...
  /**
   * Minimum interval between checks to avoid rate limiting in milliseconds (default: 5 minutes)
   */
...
/**
 * Dependencies configuration - maps remote names to zephyr: protocol strings
 * Format: { RemoteName: "zephyr:appName.projectName.orgName@environment" }
 */
