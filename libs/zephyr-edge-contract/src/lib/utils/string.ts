 ...
 */
export function formatString<const S extends string>(str: S, params: Record<FindTemplates<S>, string | number | boolean>): string {
...
export type FindTemplates<S extends string> = `${S}` extends `${infer Prefix}{{ ${infer Key} }}${infer Suffix}`
