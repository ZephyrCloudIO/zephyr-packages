...
import { blackBright, blue, bold, cyanBright, underline, whiteBright } from '../utils/picocolor';
import { FindTemplates, formatString } from '../utils/string';
import { stripAnsi } from '../utils/strip-ansi';
import { isZeErrorEqual, ZeErrorCategories, ZeErrorCode, type ZeErrorCodes, type ZeErrorKeys, ZeErrors, ZeErrorType } from './codes';
 ...
 */
export class ZephyrError<K extends ZeErrorKeys, T extends (typeof ZeErrors)[K] = (typeof ZeErrors)[K]> extends Error {
  ...
  static fromZeCode(code: ZeErrorCodes): ZeErrorType {
    ...
    for (const errorCategory of Object.keys(ZeErrorCategories) as (keyof typeof ZeErrorCategories)[]) {
  ...
  static format(error: unknown): string {
    const zeError = ZephyrError.is(error)
      ? error
      : new ZephyrError(ZeErrors.ERR_UNKNOWN, { message: (error as any)?.message || String(error), cause: error });
...
`.trim(),
      ...
      tmpFile && blackBright(`Complete error details available at ${whiteBright(tmpFile)}`),
    ];
