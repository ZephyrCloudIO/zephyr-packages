 ...
 * 
 * Contains the main logic for converting Next.js outputs to Zephyr format,
 * creating snapshots, and uploading to Zephyr Cloud.
 */
...
export async function convertToZephyrAssets(
  ctx: BuildContext, 
  log: ReturnType<typeof import('./utils').createLogger>
...
export async function createSnapshot(
  ...
  const snapshot: ZephyrSnapshot = {
    ...
    metadata: {
      ...
      hasAPIRoutes: Array.from(zephyrAssets.serverFunctions.values()).some(asset => 
        asset.type === 'APP_ROUTE' || asset.type === 'PAGES_API'
      ) || Array.from(zephyrAssets.edgeFunctions.values()).some(asset => 
        asset.type === 'APP_ROUTE' || asset.type === 'PAGES_API'
      ),
      hasSSR: Array.from(zephyrAssets.serverFunctions.values()).some(asset => 
        asset.type === 'APP_PAGE' || asset.type === 'PAGES'
      ),
...
async function simulateDirectUpload(
  ...
  await simulateUpload('Public assets', snapshot.deploymentTargets.cdn.publicAssets.length, log)
  
  log.info('⚡ Uploading Edge functions...')
  await simulateUpload('Edge functions', snapshot.deploymentTargets.edge.functions.length, log)
  
  log.info('🖥️  Uploading Server functions...')
  await simulateUpload('Server functions', snapshot.deploymentTargets.server.functions.length, log)
...
async function saveSnapshotManifest(
  ...
  try {
    ...
    log.warn('⚠️  Could not save snapshot manifest:', error instanceof Error ? error.message : 'Unknown error')
  }
}