...
import { logFn, ZephyrEngine, ZephyrError, ze_log } from 'zephyr-agent';
...
import { extractVitePreloadImports } from './internal/mf-vite-etl/preload-imports';
import type { OutputBundle, ProgramNode } from 'rollup';
import type { ApplicationConsumes, UsedIn } from 'zephyr-edge-contract';
import { parseRemoteMapAndImportedRemotes } from './internal/mf-vite-etl/remote_map_parser';
  ...
  return {
    ...
    closeBundle: async () => {
      try {
        ...
        ze_log('vite.closeBundle.consumes: ', consumes);
        ...
        const buildStats = await extractViteBuildStats({
          ...
          consumes, // Pass the imports we've already discovered
        });
