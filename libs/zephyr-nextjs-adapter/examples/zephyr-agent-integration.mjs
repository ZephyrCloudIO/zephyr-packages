 ...
 *
 * This example shows how to integrate the adapter with existing Zephyr Agent
 * infrastructure for seamless compatibility.
 */
  ...
  onBuildComplete: async (ctx) => {
    ...
    try {
      ...
      const { createSnapshot } = await import(
        'zephyr-agent/lib/transformers/ze-build-snapshot'
      );
      ...
      console.log(
        `📊 Converted ${ctx.outputs.length} outputs to Zephyr format`
      );
